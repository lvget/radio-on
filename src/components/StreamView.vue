<template>
  <div style="width: 90%; max-width: 500px; margin: 0 auto;">
    ...
    <q-card class="bg1 q-mb-md">
      <q-card-section>
        <div class=text-h6>Radio: {{ AudioLibrary.currentStream.name }}</div>
        <div class="ellipsis text-subtitle2">{{ AudioLibrary.currentStream.src }}</div>
      </q-card-section>
      <q-card-section>
      ...
      <q-card-section>
        ...
        <div>
          ...
          <div v-if="streamData.file">
            <q-btn flat icon="las la-download" :href="streamData.file" target="_blank" label="Download track" />
          </div>
